...
#------------------------------------------------------------------------
# This program is modified by Shiroyama and MEDIADO Infracture base on AWS source code.
# Please contact with tech_infra@mediado.infor for more information.
...
my $load_average_option_1min;
my $load_average_option_5min;
...

{
  ...
  $parse_result = GetOptions(
    ...
    'load-average-1min' => \$load_average_option_1min,
    'load-average-5min' => \$load_average_option_5min,
...
if (!$report_mem_util && !$report_mem_used && !$report_mem_avail
  && !$report_swap_util && !$report_swap_used && !$report_disk_space && !$load_average_option_1min && !$load_average_option_5min)
{
...
# collect load average metrics (1min)

if ($load_average_option_1min) {
    my $load_average_1min = __get_load_average_1min();
    add_metric('1MinLoadAverage', 'Count', $load_average_1min);
}

sub __get_load_average_1min {
    my $uptime = `uptime`;
    return unless $uptime;
    chomp($uptime);

    die unless ($uptime =~ /\A.*load\s?averages?:\s?(.*)\z/);
    my $onemin_load_average = $1;
    ...
    my @load_averages_1min = split(/$delimiter/, $onemin_load_average);

    return wantarray ? @load_averages_1min : shift @load_averages_1min;
}

# collect load average metrics (5min)

if ($load_average_option_5min) {
    my $load_average_5min = __get_load_average_5min();
    add_metric('5MinLoadAverage', 'Count', $load_average_5min);
}

sub __get_load_average_5min {
    my $uptime = `uptime`;
    return unless $uptime;
    chomp($uptime);

    die unless ($uptime =~ /\A.*load\s?averages?:\s?(.*)\z/);
    my $fivemin_load_average = $1;
    ...
    my $delimiter = $type_to_delimiter{ lc($os_type) } || $type_to_delimiter{'other'};
    my @load_averages_5min = split(/$delimiter/, $fivemin_load_average);
    shift @load_averages_5min;

    return wantarray ? @load_averages_5min : shift @load_averages_5min;
}
